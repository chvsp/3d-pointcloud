...
{
  if (fabs (point_a.intensity - point_b.intensity) > 30.0f)
    return (false);
  else
    return (true);
}
...
main (int argc, char** argv)
{
  ...
  {
    clustercounter += (*clusters)[i].indices.size ();
  }

  pcl::PointCloud<pcl::PointXYZI>::Ptr cloudclus (new pcl::PointCloud<pcl::PointXYZI> ());
  cloudclus->width  = 5;
  cloudclus->height = 1;
  cloudclus->points.resize (cloudclus->width * cloudclus->height);
  ...
  pcl::PCDWriter writerfin;
  std::stringstream ss;
  ss << "cloud_filtered_p"  << ".pcd";
  writerfin.write<pcl::PointXYZI> (ss.str (), *cloudclus, false); //*
  pcl::io::savePCDFileASCII (ss.str(), *cloudclus);
